...
class DbHelper {
  ...
  _onCreate(Database db, int intVersion) async {
    await db.execute("CREATE TABLE $Table_User ("
        ...
        ")");
    await db.execute("CREATE TABLE $Table_Book ("
        " $Book_ID TEXT, "
        " $Book_Name TEXT, "
        " PRIMARY KEY ($Book_ID)"
        ")");
  ...
  Future<int> saveBook(BookModel book) async {
    var dbClient = await db;
    var res = await dbClient.insert(Table_Book, book.toMap());
    return res;
  }
  ...
  Future<int> deleteBook(String book_id) async {
    var dbClient = await db;
    var res = await dbClient
        .delete(Table_Book, where: '$Book_ID = ?', whereArgs: [book_id]);
    return res;
  }
}
